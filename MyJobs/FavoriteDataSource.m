//
//  FavoriteDataSource.m
//  JobLink
//
//  Created by Joji Kubota on 5/3/15.
//  Copyright (c) 2015 Joji Kubota. All rights reserved.
//

#import "FavoriteDataSource.h"

@interface FavoriteDataSource()

@property (nonatomic) Job *aJob;
@property (nonatomic) NSArray *parseJobs;
@property (nonatomic) NSMutableArray *jobs;


@end

@implementation FavoriteDataSource

-(instancetype) init {
    if ((self = [super init]) == nil) {
        return nil;
    }
    
    /* Initialize the arrays. */
    self.jobs = [[NSMutableArray alloc] init];
    self.parseJobs = [[NSArray alloc] init];
    
    /* Pull the favorites from Parse for the user */
    PFQuery *query = [PFQuery queryWithClassName:@"FavJobs"];
    if (!query) {
        NSString *usernmae = [PFUser currentUser].username;
        [query whereKey:@"user" equalTo:usernmae];
        [query orderByDescending:@"createdAt"];
        [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
            if (objects) {
                self.parseJobs = objects;
            }
            else {
                NSLog(@"No favorites %@", error);
            }
         }];
    }
    
    /* Save back the data into Job object */
    for (PFObject *job in self.parseJobs) {
        self.aJob.jobtitle = job[@"jobtitle"];
        self.aJob.company = job[@"company"];
        self.aJob.city = job[@"city"];
        self.aJob.state = job[@"state"];
        self.aJob.snippet = job[@"snippet"];
        self.aJob.url = job[@"url"];
        self.aJob.formattedRelativeTime = job[@"formattedRelativeTime"];
        self.aJob.datePosted = job[@"datePosted"];
        self.aJob.isFav = job[@"isFav"];
        self.aJob.score = job[@"score"];
        self.aJob.skillsList = job[@"skillsList"];
        self.aJob.sourceType = job[@"sourceType"];
        [self.jobs addObject:job];
    }
    
    NSLog(@"size of the array is %d", [self.jobs count]);
    
    return self;
}

- (NSMutableArray *) getAllJobs {
    return self.jobs;
}

- (NSInteger *) getNumberOfJobs {
    return (NSInteger *)[self.jobs count];
}

- (Job *) jobAtIndex: (NSInteger *) idx {
    if( idx >= (NSInteger *)[self.jobs count] )
        return nil;
    
    return [self.jobs objectAtIndex: (int)idx];
}

@end
